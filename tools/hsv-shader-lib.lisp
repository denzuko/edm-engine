...
(defun write-generated-shader-source (output-path forms &key generator-script description-lines)
  "Writes FORMS (c-mera source forms) to OUTPUT-PATH with a standard
header comment (GENERATED-BY note, DESCRIPTION-LINES verbatim, a
regenerate hint) — the actual, real duplication two separate
generator scripts used to each hand-roll themselves (identical
WITH-OPEN-FILE/PRIN1/TERPRI boilerplate, only the path/forms/comment
text differing), now composed here instead. DESCRIPTION-LINES is each
generator's own genuinely different, meaningful explanation of what
it generates — not boilerplate to deduplicate further, passed through
verbatim."
  (ensure-directories-exist output-path)
  (with-open-file (out output-path :direction :output
                                    :if-exists :supersede :if-does-not-exist :create)
    (format out ";;;; GENERATED by ~A — do not hand-edit.~%" generator-script)
    (dolist (line description-lines)
      (format out ";;;; ~A~%" line))
    (format out ";;;; Regenerate: sbcl --script ~A~%~%" generator-script)
    (dolist (form forms)
