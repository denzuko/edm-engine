   ...
   (lambda ()
     (let* ((p (hearts-game-turn game))
       ...
       (multiple-value-bind (sx sy) (ai-origin-position p (float window-width 1.0) (float window-height 1.0))
         (start-card-tween card sx sy (trick-card-x window-width trick-index) (trick-card-y window-height)))
              ...
              (when (raylib:is-key-pressed :key-enter)
                (let* ((card (nth (hearts-game-cursor game) hand))
                  ...
                  (when (member card legal :test #'equal)
                    (start-card-tween card (hand-card-x (hearts-game-cursor game)) (hand-card-y window-height)
                                       (trick-card-x window-width (length (hearts-game-current-trick game)))
                                       (trick-card-y window-height))
